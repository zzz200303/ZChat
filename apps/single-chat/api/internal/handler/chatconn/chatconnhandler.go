...
func (n *Node) GetOfflineMessage() {
	offlineMessage, err := n.SvcCtx.Redis.Lrange(fmt.Sprintf("%s::%d", constants.OFFLINE_MESSAGE, n.Uid), 0, -1)
	// 从Redis获取离线消息
	//生成一个 Redis 列表的键，格式为 utils.OFFLINE_MESSAGE::用户ID，例如 OFFLINE_MESSAGE::12345。
	...
	// 删除Redis中的离线消息键
	_, err = n.SvcCtx.Redis.Del(fmt.Sprintf("%s::%d", constants.OFFLINE_MESSAGE, n.Uid))
	if err != nil {
