...
func (l *GroupPutinLogic) GroupPutin(in *social.GroupPutinReq) (*social.GroupPutinResp, error) {
	...
	if err != nil && !errors.Is(err, models.ErrNotFound) {
		return nil, errors.Wrapf(err, "find group member by groud id and  req id err %v, req %v, %v", err,
			in.GroupId, in.ReqId)
	}
	...
	if err != nil && err != models.ErrNotFound {
		return nil, errors.Wrapf(err, "find group req by groud id and user id err %v, req %v, %v", err,
			in.GroupId, in.ReqId)
	}
	if groupReq != nil {
		return &social.GroupPutinResp{}, nil
	...
	if err != nil {
		return nil, errors.Wrapf(err, "find group by groud id err %v, req %v", err, in.GroupId)
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "find group member by groud id and user id err %v, req %v",
...
func (l *GroupPutinLogic) createGroupReq(groupReq *models.GroupRequests, isPass bool) (*social.GroupPutinResp, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "insert group req err %v req %v", err, groupReq)
	}
...
func (l *GroupPutinLogic) createGroupMember(in *social.GroupPutinReq) error {
	...
	_, err := l.svcCtx.GroupMembersModel.InsertWithSession(l.ctx, nil, groupMember)
	if err != nil {
